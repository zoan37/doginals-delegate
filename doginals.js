...
function inscribe(wallet, address, contentType, data, delegateTxId = "") {
    // Add this right at the start of the inscribe function
    console.log('Wallet UTXOs details:', JSON.stringify(wallet.utxos, null, 2));

    // Add debug logging
    console.log('Starting inscription with:');
    console.log('Address:', address.toString());
    console.log('Delegate TxId:', delegateTxId);
    console.log('Wallet UTXOs:', wallet.utxos.length);
    ...
    if (delegateTxId.length > 0) {  // in case we want to inscribe a delegate
        console.log('Creating delegate inscription');
        ...
        // Debug the delegate txid conversion
        console.log('Converting delegate TxId:', delegateTxId);
        let chunk = IdToChunk(delegateTxId);
        console.log('Converted chunk:', chunk);
        inscription.chunks.push(chunk);
    ...
    while (inscription.chunks.length) {
        ...
        console.log('Created p2shOutput with satoshis:', p2shOutput.satoshis);
